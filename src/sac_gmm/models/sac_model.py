"""
Lightning module para SAC puro (sin GMM).

Implementación gemela de SACGMM (src/sac_gmm/models/sac_gmm_model.py) pero
adaptada al SACAgent (sac_gmm/agents/calvin/sac_agent.py):

  1. SACAgent.play_step()  -> recibe (actor, strategy, replay_buffer)
                              (SACGMMAgent también acepta `device` pero
                              SACAgent no, por eso no se lo pasamos).
  2. SACAgent.evaluate()   -> retorna (accuracy, return, length, video_path)
                              (SACGMMAgent retorna 3 valores).

El resto de la mecánica (losses, soft update, alpha) es exactamente igual.
"""
import logging
...
class SAC(SkillModel):
    """SAC puro implementado con PyTorch Lightning."""
    ...
    def training_step(self, batch, batch_idx):
        # SACAgent.play_step solo recibe 3 args (sin device)
    ...
    def on_train_epoch_end(self):
        ...
        if self.episode_done:
            ...
            if self.episode_idx % self.eval_frequency == 0:
                # SACAgent.evaluate retorna 4-tuple (extra video_path)
                eval_out = self.agent.evaluate(self.actor)
                if len(eval_out) == 4:
                    eval_accuracy, eval_return, eval_length, _ = eval_out
                else:
                    eval_accuracy, eval_return, eval_length = eval_out
                eval_metrics = {
    ...
    def compute_critic_loss(self, batch, features, critic_optimizer):
        ...
        with torch.no_grad():
            ...
            policy_actions, log_pi = self.actor.get_actions(
                next_state, deterministic=False, reparameterize=False
            )
            ...
            q_target = batch_rewards + (1 - batch_dones) * self.discount * (
                q_next_target - self.alpha * log_pi
            )
    ...
    def compute_actor_and_alpha_loss(self, batch, features, actor_optimizer, alpha_optimizer):
        ...
        policy_actions, log_pi = self.actor.get_actions(
            state, deterministic=False, reparameterize=True
        )
